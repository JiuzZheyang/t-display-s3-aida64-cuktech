...
static DeviceConfig _cfg_buf;
static DeviceConfig *_cfg = &_cfg_buf;
...
static const char *MD5_HEX[NUM_DEVICES] = {
    "d0dbbf2e94cfbbb94890171831115c58",
    "58ba80e67dd21f0f59b44d78d5567297",
    "cf5fe9b80b6039d0bd702d9d8439bc3c",
    "67e92b6bb3e2057d1cdc1829d1e6a7c4",
    "2fc0d9b7387e677f287fbdba07f2877a",
};

static const char *PORT_NAMES[5] = {"c1", "c2", "c3", "a", "ble"};
...
#define INTERVAL_PING_SEND  30
#define INTERVAL_PING_RECV  20
#define MAX_PING_LOST       3
#define GRACE_PERIOD_SEC    3

static uint64_t _last_ping_time = 0;
static uint64_t _ping_send_time = 0;
static int _ping_lost = 0;
static bool _ping_waiting = false;
static bool _grace_active = false;
...
static void _mqtt_event_handler(void *arg, esp_event_base_t base, int32_t id, void *data) {
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        for (int i = 0; i < NUM_DEVICES; i++) {
            if (_topics[i][0]) {
                ...
                esp_mqtt_client_subscribe(_client, _pub_topics[i], 1);  /* 同时订阅 /set 主题 */
        ...
        for (int i = 0; i < 4; i++) {
            ...
            bool st = _port_state[i];
            portEXIT_CRITICAL(&_state_mux);
            esp_mqtt_client_publish(_client, _pub_topics[i], st ? "on" : "off", 0, 1, 1);
        }
        {
            portENTER_CRITICAL(&_state_mux);
            bool st = _ble_state;
            portEXIT_CRITICAL(&_state_mux);
            esp_mqtt_client_publish(_client, _pub_topics[4], st ? "on" : "off", 0, 1, 1);
        }
        break;
    ...
    case MQTT_EVENT_DISCONNECTED:
        ...
        if (_connect_fail_count >= MAX_CONNECT_FAILS) {
            ESP_LOGE(TAG, "Circuit breaker open: %d fails", _connect_fail_count);
            _circuit_open = true;
    ...
    case MQTT_EVENT_DATA: {
        ...
        strncpy(match_topic, topic_buf, sizeof(match_topic) - 1);
        match_topic[sizeof(match_topic) - 1] = '\0';
        char *slash = strrchr(match_topic, '/');
        if (slash && strcmp(slash, "/set") == 0) *slash = '\0';
        for (int i = 0; i < NUM_DEVICES; i++) {
            if (_topics[i][0] && strcmp(match_topic, _topics[i]) == 0) {
                bool ok = false;
                if (i == 4 && _on_ble) {
                    ok = _on_ble(on);
                } else if (_on_cmd) {
...
void bemfa_init(const DeviceConfig *cfg, bemfa_cmd_cb on_cmd, bemfa_ble_cb on_ble) {
    ...
    _on_ble = on_ble;
    _enabled = true;
    memcpy(_cfg, cfg, sizeof(DeviceConfig));
    ...
    /* 禁用HTTP注册 — 直接在巴法云后台手动创建topic即可 */
    // xTaskCreate(_register_task, "bemfa_reg", 4096, NULL, 1, NULL);
    ESP_LOGI(TAG, "Skipping HTTP registration (manual topic creation)");

    char uri[128];
    snprintf(uri, sizeof(uri), "mqtt://bemfa.com:9501");

    /* 使用配置文件中的 AppID 和 SecretKey，如果为空则尝试用 UID 方式连接 */
    const char *mqtt_user = cfg->bemfa_appid[0] ? cfg->bemfa_appid : cfg->bemfa_uid;
    const char *mqtt_pass = cfg->bemfa_secret[0] ? cfg->bemfa_secret : "";
...
void bemfa_publish_port(int idx, float voltage, float current, float power, bool active) {
    (void)voltage; (void)current; (void)power;
    if (!_enabled || !_client || !_connected || idx < 0 || idx >= 4) return;
    portENTER_CRITICAL(&_state_mux);
    _port_state[idx] = active;
    portEXIT_CRITICAL(&_state_mux);
    esp_mqtt_client_publish(_client, _pub_topics[idx], active ? "on" : "off", 0, 1, 1);
}
...
void bemfa_publish_status(bool connected) {
    ...
    portEXIT_CRITICAL(&_state_mux);
    esp_mqtt_client_publish(_client, _pub_topics[4], connected ? "on" : "off", 0, 1, 1);
}
