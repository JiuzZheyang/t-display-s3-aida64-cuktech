...
void config_store_init(void) {
    /* NVS flash 已在 app_main 中初始化，此处无需操作 */
}
...
bool config_store_load(DeviceConfig *cfg) {
    ...
    _str(h, "bemfa_appid", cfg->bemfa_appid, sizeof(cfg->bemfa_appid));
    _str(h, "bemfa_secret", cfg->bemfa_secret, sizeof(cfg->bemfa_secret));
    ESP_LOGI(TAG, "Loaded bemfa: enable=%d uid='%s' appid='%s'", cfg->bemfa_enable, cfg->bemfa_uid, cfg->bemfa_appid);
    // bemfa_token 和 bemfa_topic 已移除 — 只需要 UID
...
bool config_store_save(const DeviceConfig *cfg) {
    ...
    /* 先删除再写入，避免NVS碎片 */
    nvs_erase_key(h, "bemfa_uid");
    nvs_set_str(h, "bemfa_uid", cfg->bemfa_uid);
    nvs_set_str(h, "bemfa_appid", cfg->bemfa_appid);
    nvs_set_str(h, "bemfa_secret", cfg->bemfa_secret);
    ESP_LOGI(TAG, "Saving bemfa: enable=%d uid='%s'", cfg->bemfa_enable, cfg->bemfa_uid);
